...
== Interface:
There is a basic interface that exists at the class level for loading nodes and related nodes
...
=== Lazy Loading:
By default Deja supports lazy loading. To load a given relationship on the fly, simply call the relationship method. If the relationship does not yet exist, it will be fetched from the graph. 
